...
// Strategy Builder: Frontend 3010 -> Backend 8010 (via Next.js rewrites proxy)
...
function formatApiErrorMessage(statusCode: number, errorText: string): string {
  const fallback = errorText || `API request failed (${statusCode})`;
  try {
...
async function handleResponse<T>(response: Response): Promise<T> {
  if (!response.ok) {
    ...
    throw new ApiError(response.status, formatApiErrorMessage(response.status, errorText));
  }
