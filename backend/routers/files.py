...
전략 파일 Import/Export (.kb.yaml, .py)
- 외부 포맷 의존성 제거됨
...
async def download_template(template_id: str) -> StreamingResponse:
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="{template_id}.kb.yaml"'
        },
    )
...
async def export_strategy(
    ...
    if format == "yaml":
        ...
        return StreamingResponse(
            ...
            media_type="application/x-yaml",
            headers={
                "Content-Disposition": f'attachment; filename="{filename}.kb.yaml"'
            },
        )
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
            zf.writestr(f"{filename}.kb.yaml", yaml_content)
